...
def build_app(config_path=None):
    ...
    with gr.Blocks(title="SAM2 Prompter") as app:
        ...
        # Automatically load the default folder on UI start
        app.load(
            load_folder,
            inputs=[f_dd],
            outputs=[image_display, folder_path_state, image_files_state, frame_slider, status_label, dots_by_frame_state]
...
if __name__ == "__main__":
    ...
    # Using local host instead of 0.0.0.0 due to default dev behavior, but we'll respect origin/dev settings.
